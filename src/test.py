...
def eval_language_metrics(checkpoint, eval_data_loader, opt, model=None, eval_mode="test"):
    ...
    if opt.dset_name == "anet":
        ...
        reference_files_map = {"test": [os.path.join("our_yc2_data", "yc2_test_anet_format_para.json")]}
...
def main():
    ...
    val_dataset = RCDataset(
        ...
        max_v_len=opt.max_v_len, max_n_sen=opt.max_n_sen+10, mode="test",
    ...
    opt.vocab_size = len(load_json(opt.word2idx_path)) #len(train_dataset.word2idx)
    ...
    if opt.xl:
        checkpoint = torch.load("/mnt/LSTA5/data/nishimura/svpc_pp_gt/baseline/xl/xl.chkpt")
    else:
        checkpoint = torch.load("/mnt/LSTA5/data/nishimura/svpc_pp_gt/baseline/mart/mart.chkpt")

    print("[Info] Loaded weights")
