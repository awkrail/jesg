...
class Translator(object):
    ...
    def translate_batch_greedy(self, video_features, event_features, event_abs_features,
                               video_masks, max_index, text_input_ids, text_masks, text_input_labels, tiou_scores, rt_model):
        def greedy_decoding_step(v_prev_ms, t_prev_ms, video_feats, event_feats, event_abs_feats,
                                 v_masks, max_idx, text_input_id, text_mask, text_input_label, tiou_score, model,
            ...
            selected_event_feats, attn_max_indices = model.select_event(video_prev_ms, encoder_layer_outputs[-1], max_idx, 
                                                                        event_abs_features=None, gt_timestamps=None, tiou_scores=None, inference=True)
        ...
        with torch.no_grad():
            ...
            for idx in range(step_size):
                video_prev_ms, text_prev_ms, dec_seq, selected_event_indices = greedy_decoding_step(
                        ...
                        text_input_labels[idx], tiou_scores[idx], rt_model, 
                        config.max_v_len, config.max_t_len)
    ...
    def translate_batch_single_sentence_untied_greedy(
            cls, video_features, event_features, video_masks, max_index,
            text_input_ids, text_masks, text_input_labels, tiou_scores, model, 
            start_idx=RCDataset.BOS, unk_idx=RCDataset.UNK):
        """The first few args are the same to the input to the forward_step func
        Note:
            1, Copy the prev_ms each word generation step, as the func will modify this value,
            ...
            next memory state tensor.
        """
        encoder_outputs = model.encode(video_features, video_masks, event_features)  # (N, Lv, D)
        selected_event_feats, selected_event_indices = model.select_event(encoder_outputs, max_index, tiou_scores, inference=True)
        ...
        for dec_idx in range(max_t_len):
            ...
            next_words = pred_scores[:, dec_idx].max(1)[1]
            next_symbols = next_words
        return text_input_ids, selected_event_indices  # (N, Lt)
    ...
    def translate_batch(self, model_inputs, use_beam=False, recurrent=True, untied=False, xl=False, mtrans=False):
        ...
        if use_beam:
            if recurrent:
                video_features, event_features, video_masks, max_index, text_input_ids, text_masks, text_input_labels, tiou_scores = model_inputs
            ...
            if recurrent:
                video_features, event_features, video_masks, event_abs_features, \
                        max_index, text_input_ids, text_masks, text_input_labels, tiou_scores = model_inputs
                if xl:
                    ...
                    return self.translate_batch_greedy(
                        ...
                        text_input_ids, text_masks, text_input_labels, tiou_scores, self.model)
            else:  # single sentence
                if untied or mtrans:
                    video_features, event_features, video_masks, max_index, text_input_ids, text_masks, text_input_labels, tiou_scores = model_inputs
                    return self.translate_batch_single_sentence_untied_greedy(
                        video_features, event_features, video_masks, max_index,
                        text_input_ids, text_masks, text_input_labels, tiou_scores, self.model)
                else:
